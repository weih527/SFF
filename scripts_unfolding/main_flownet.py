...
from loss.multiscaleloss import multiscaleEPE, realEPE
...
def loop(cfg, train_provider, valid_provider, model, criterion, optimizer, iters, writer):
    ...
    while iters <= cfg.TRAIN.total_iters:
        ...
        loss = multiscaleEPE(output, target, weights=multiscale_weights, sparse=sparse)
        flow2_EPE = div_flow * realEPE(output[0], target, sparse=sparse)
        loss.backward()
        ...
        if iters % cfg.TRAIN.display_freq == 0 or iters == 1:
            ...
            writer.add_scalar('loss/loss', sum_loss / cfg.TRAIN.display_freq * 1, iters)
            writer.add_scalar('loss/EPE', flow2_EPE, iters)
        ...
        if iters % cfg.TRAIN.save_freq == 0 or iters == 1:
            ...
            for k, data in enumerate(dataloader, 0):
                ...
                loss_valid = div_flow * realEPE(output, gt, sparse=sparse)
                b, _, h, w = gt.size()
                pred = F.interpolate(output, (h,w), mode='bilinear', align_corners=False)
